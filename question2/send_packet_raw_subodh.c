...
// #define destination_ip 10.240.253.10
 #define destination_ip 192.168.0.102
...
void get_eth_index()
{
	...
	strncpy(ifreq_i.ifr_name,"wlp5s0",IFNAMSIZ-1);
...
void get_mac()
{
	...
	strncpy(ifreq_c.ifr_name,"wlp5s0",IFNAMSIZ-1);
...
void get_ip()
{
	...
	strncpy(ifreq_ip.ifr_name,"wlp5s0",IFNAMSIZ-1);
...
int main()
{
	...
	//  send_len = send(sock_raw,sendbuff,64,0);
		if(send_len<0)
		{
