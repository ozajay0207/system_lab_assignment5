...
int advertise_request(char IP[]){
	...
	// SET DESTINATION ADDRESS AS BROADCAST FOR ETHERNET FRAME
...
int wait_for_reply(){
	...
	while(1)
	{
		...
		while (((((ethernet_frame[12]) << 8) + ethernet_frame[13]) != ETH_P_ARP) || (ntohs (arphdr->opcode) != ARP_REPLY_OP) || arphdr->temp !=1) {
...
int main (int argc, char **argv)
{
	...
	//SEND THE BROADCAST REQUEST PACKET AT SOME RANDOM IP
	//SET UP THE DESTINATION IP
	strcpy(neighbour_ip,"192.168.0.2");
	//strcat(neighbour_ip,temp);
	printf("Broadcast IP (random) :%s\n",neighbour_ip);
	...
	printf("Advertise Request Sent\n");
	printf("\nWaiting for the reply from Neighbours...\n");
